...
KCSNBShiny<-function()
{
    ...
    sidebarLayout(
      ...
      mainPanel(
        tabsetPanel(
          ...
          tabPanel(title = "Test Results And Accuracy",
                   tags$h4("Showing the top six rows of the Actual and Predicted Values"),
  ...
  server <- function(input, output, session) {
    ...
    output$Apriori<-renderPrint({
      ...
      TitanicNB<-naiveBayes(get(input$var) ~ .,data=train.df)
      TitanicNB
    })
